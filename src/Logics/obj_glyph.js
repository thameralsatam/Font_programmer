...
function Glyph(oa) {
	...
	this.rightsidebearing = isval(oa.rightsidebearing)
		? oa.rightsidebearing
		: false;
...
Glyph.prototype.updateGlyphSize = function (
	dw,
	dh,
	ratiolock,
	dontscalecomponentinstances
) {
	...
	for (var i = 0; i < this.shapes.length; i++) {
		if (this.shapes[i].objtype === 'componentinstance') {
			this.shapes[i].rotate(
				deltaRad - niceAngleToRadians(this.shapes[i].rotation),
				about,
				snap
			);
		} else {
...
Glyph.prototype.getMaxes = function () {
	...
	if (this.shapes.length) {
		if (
			this.maxes.xmin === _UI.maxes.xmin ||
...
Glyph.prototype.copyShapesTo = function (
	destinationID,
	copyGlyphAttributes,
	selectNewShapes
) {
...
function getGlyph(id, create) {
	...
	if (!_GP || Object.keys(_GP).length === 0) {
...
function getSelectedGlyphLeftSideBearing() {
	...
	if (sc.leftsidebearing === true)
		sc.leftsidebearing = _GP.projectsettings.defaultlsb;
	return sc.leftsidebearing !== false
		? sc.leftsidebearing
		: _GP.projectsettings.defaultlsb;
}
...
function getSelectedGlyphRightSideBearing() {
	...
	if (sc.rightsidebearing === true)
		sc.rightsidebearing = _GP.projectsettings.defaultrsb;
	return sc.rightsidebearing !== false
		? sc.rightsidebearing
		: _GP.projectsettings.defaultrsb;
}
...
function updateCurrentGlyphWidth() {
	...
	if (_UI.current_page === 'glyph edit') {
		...
		var lsarr = sc.usedin;
		if (lsarr)
			for (var c = 0; c < lsarr.length; c++) getGlyph(lsarr[c]).changed();
	}
}
